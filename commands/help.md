...
bengo-toolkit には 24 コマンドがあるが、弁護士が覚えるべきなのは `/help` ただ 1 つ。
...
- **`--all`** → 24 コマンド全件（Cowork では blocked に "Claude Code 限定" 印）
...
【メンテナンス】
  ...
  /verify            動作確認
  /report-issue      不具合報告・機能要望をフォームから送る（GitHub 不要）
...
| 「バグ」「不具合」「フィードバック」「要望」 | `/report-issue` |
