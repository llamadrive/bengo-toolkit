...
- `--scope firm` の場合: `/template-firm-setup` で設定したローカル同期フォルダ
- `--scope user` の場合: `~/.claude-bengo/templates/`

$ARGUMENTS: XLSXファイルのパス（任意。なければ対話で確認）。

フラグ:
- `--scope case`   — この案件フォルダのみ（**既定**）。`<workspace>/.claude-bengo/templates/`。
- `--scope firm`   — 事務所全員で共有（要 `/template-firm-setup`）。PII 検出時は拒否される。
- `--scope user`   — この端末・lawyer 限定。`~/.claude-bengo/templates/`。PII 検出時は拒否される。
