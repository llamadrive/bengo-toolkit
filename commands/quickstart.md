...
出力は **そのままユーザーへ verbatim に表示する**。Cowork 環境ではローカル
ファイルを必要とするデモ（戸籍解析・テンプレート入力・校正・訴訟分析）が
自動的に除外され、条文検索と相続分計算のみが表示される。
...
**fixtures の絶対パスは `${CLAUDE_PLUGIN_ROOT}` から組み立てる。**
Claude Code はスラッシュコマンド実行時にこの env var をプラグインの install
ディレクトリに設定する（marketplace cache であれ dev リンクであれ）。
`ls` で `~/.claude/plugins/cache/.../bengo-toolkit/<ver>/` を探索しない。

```bash
# 番号 1（家系図）の例:
FIXTURE="${CLAUDE_PLUGIN_ROOT}/fixtures/family-tree/koseki-simple.pdf"
```

`${CLAUDE_PLUGIN_ROOT}` が未定義（旧 Claude Code 等）の場合のみ、
`find ~/.claude/plugins/cache -maxdepth 5 -name "bengo-toolkit" -type d` を
1 度だけ実行して `<...>/<version>/` の最新版を採用する。

**初期化は並列に実行する。** 番号 1-4 はいずれも以下を行うため、独立な
ステップを 1 メッセージ内で同時に呼ぶ:

- `mktemp -d -t cbengo-demo` の作成
- `workspace.py init --cwd <tmp>` の初期化
- 入力 fixture の Read（PDF）または stat（DOCX/XLSX）

`audit.py record` は workspace 初期化に依存するため init 完了後に並列で
呼ぶ。Read（vision PDF）と audit record は順序依存しないので同一メッセージ
内で並列化する。
