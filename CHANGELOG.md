...
- **`/typo-check` が原本 DOCX を直接書き換えていた不具合を修正。原本を編集せず、まず
  複製 `{元ファイル名}_reviewed.docx` を作り、以降の校正・コメント付与をすべて複製に対して
  行うようにした。** 従来は `edit_paragraph` 等を原本パスに直接適用しており、修正履歴
  （track changes）付きとはいえ弁護士の手元の原本ファイル自体が改変されていた。track
  changes が無効化・破損した環境では、原本に修正が追跡なしで焼き込まれ復元手段が無くなる
  懸念があった。複製方式により原本はバイト単位で無変更に保たれる。命名規則
  （`{元ファイル名}_reviewed.docx`）は従来ドキュメント記載どおりで、実装が追従した形。
  複製先が既存の場合は上書き可否をユーザーに確認する（誤上書き防止）。

- **`/quickstart` の校正デモが、同梱サンプル `fixtures/typo-check/brief-with-errors.docx`
  を破壊していた不具合を解消。** 旧実装ではデモ実行時にサンプル原本を in-place で書き換えて
  いた。サンプルを試用フォルダへ複製してから実行するよう変更し、`fixtures/` を無変更に保つ。

**影響範囲:** DOCX を書き込むスキルは `/typo-check` のみ。`/lawsuit-analysis`・`/verify` は
DOCX を読取専用で扱うため影響なし。XLSX を扱う `/template-fill` は従来からテンプレート原本を
複製して `outputs/` に出力する方式で、原本は元から無変更。
