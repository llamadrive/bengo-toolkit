...
`memory.observations` の最初に以下の **プレーン文字列** を必ず挿入する。
...
"memory": {
  "observations": [
    "⚠ AI が訴訟文書から抽出したドラフトです。裁判所提出前に、弁護士が原本と照合して検証してください（Step 3.3 の検証ログと `ninhi_source` を参照）。"
  ],
  "preferences": {}
}
```

### Step 4: agent-format JSON（描画エンジン入力）の構築

訴訟分析レポートの成果物は **自己完結 HTML（`.html`）** とする（family-tree と同じ方針・背景は family-tree SKILL.md Step 4 を参照）。本ステップでは描画エンジンへの入力となる `.agent` JSON を組み立て、Step 5 で自己完結 HTML を生成する。`.agent` はユーザーには残さない（HTML 生成後に必ず削除する）。外部ツール連携対応ホスト（Claude Desktop / Cursor 等）では、削除前に in-chat のインライン描画にも使う。

以下の inline schema は **同梱の描画エンジン（renderer_bundle.js / agent-format 0.1.9）に準拠**する。
重要な注意（旧 v0.1.6 スナップショットからの修正点）:

- **table の列定義は `key` / `label`** を使う（旧 `id` / `name` は renderer が読まず、ヘッダ空・全セル「—」になる）。行（`rows`）は各列の `key` をそのままキーに持つ（行の `id` は行識別子として別途保持）。
- **`type: "status"` の値はオブジェクト `{ "state": "...", "comment"?: "..." }`**（文字列だとバッジにならず素テキスト）。
...
```json
{
  ...
  "sections": [
    ...
    },
    {
      ...
      "data": {
        "columns": [
          ...
          { "key": "role", "label": "役割", "type": "select", "options": ["原告", "被告", "証人", "弁護士", "裁判官", "関係者"] },
    ...
    },
    {
      ...
      "data": {
        "columns": [
          { "key": "title", "label": "主張", "type": "text" },
          { "key": "party", "label": "当事者", "type": "select", "options": ["原告", "被告"] },
          { "key": "ninhi", "label": "認否", "type": "status" },
          { "key": "supporting_points", "label": "根拠（証拠ID参照可）", "type": "text" },
          { "key": "opposing_points", "label": "反論", "type": "text" }
        ],
        "rows": [
          { "id": "a1", "title": "<主張>", "party": "原告", "ninhi": { "state": "認める" }, "supporting_points": "甲第1号証...", "opposing_points": "..." }
        ]
      }
    },
    {
      ...
      "data": {
        "columns": [
          { "key": "number", "label": "号証番号", "type": "text" },
          { "key": "party", "label": "提出当事者", "type": "select", "options": ["原告", "被告"] },
          { "key": "title", "label": "表題", "type": "text" },
          { "key": "date", "label": "作成日", "type": "date" },
          { "key": "purpose", "label": "立証趣旨", "type": "text" }
        ],
        "rows": [
...
**認否ステータスの表示:** `type: "status"` の値は **`{ "state": "認める" }`** のようにオブジェクトで渡す
（`comment` を付けると「認める · 補足」の形で併記される）。renderer は `state` の文字列を**そのままバッジのラベルとして表示**する。
色分けは `state` が `todo` / `inprogress` / `almost` / `done` / `warn` のいずれかのときのみ CSS が適用され、
それ以外（＝日本語の認否）は**中立色のチップ**になる。日本語の認否をそのまま見せたいので `{ "state": "認める" }` 等を用いる
（無理に英語 enum に寄せると表示テキストまで英語になるため避ける）。文字列を直接渡すと Cg 判定を通らず素テキストになる。

Write ツールで `lawsuit_report_{YYYY-MM-DD}.agent` として作業ディレクトリに出力する（描画エンジンへの入力。最終成果物は Step 5 の `.html`）。

### Step 5: 自己完結 HTML の生成・表示 + 監査ログ

成果物は常に `.html` の 1 ファイルとし、可能なら自動でブラウザに表示する。`.agent`
は使い捨ての描画エンジン入力で、**ユーザーには残さない**。
...
`inline`（Claude Desktop / Cursor 等）のときのみ、`render_agent_file` に
`lawsuit_report_{YYYY-MM-DD}.agent` を渡してインライン描画する。**必ず次の build
より前に行う**（build で `.agent` を削除するため）。`cli` のときはスキップする。

**HTML 生成・自動表示・内部入力の削除（全ホスト共通）:**

```bash
python3 skills/_lib/agent_html/build_html.py build --input lawsuit_report_{YYYY-MM-DD}.agent --open --prune-agent
```

出力: `lawsuit_report_{YYYY-MM-DD}.html`（作業ディレクトリに残る唯一の成果物。6 セクション: 概要メトリクス・事件概要・タイムライン・登場人物・主張と認否・証拠一覧）。`--open` は既定ブラウザで自動的に開き（開けない環境では自動でパス表示にフォールバック）、`--prune-agent` は内部入力の `.agent` を削除する。描画エンジンを inline するため依存ゼロで開け、⌘P で PDF 化でき、ネットワーク通信は行わない。

**監査ログ:**

```bash
python3 skills/_lib/audit.py record --skill lawsuit-analysis --event file_write --file "lawsuit_report_{YYYY-MM-DD}.html"
```

**ユーザーへの案内:**

```
訴訟分析レポートを lawsuit_report_{YYYY-MM-DD}.html に出力し、ブラウザで開いた。

  ・特別なソフトは不要。自動で開かなかった場合もダブルクリックで開ける
    （6 セクション: 概要メトリクス・事件概要・タイムライン・登場人物・主張と認否・証拠一覧）。
  ・PDF 化は開いたページで ⌘P（Windows は Ctrl+P）→「PDF として保存」。
  ・外部に通信しないため、メールにそのまま添付して共有できる。
```
