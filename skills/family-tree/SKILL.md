...
2. **尊属（祖父母世代）の扱い**
   ...
   - どちらの書式も **`variant: "jp-court"`** を使う。Japanese 法定
...
**重要:** `section.type` は **`family-graph`** を使う。SPEC § 4 により renderer は `data.persons` の全員を必ず描画する — 書式（標準/拡張）の差は**データに何を含めるか**で表現する。
