#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""bengo-toolkit 監査ログ記録モジュール（v3.0.0 workspace-based 設計）。

本モジュールは、本プラグインで処理した文書のメタデータ（タイムスタンプ・
スキル名・ファイル名 SHA-256・バイト数・チェーンハッシュ等）を追記専用
JSONL として記録する。

## 法的位置付けに関する注記

本ログは、個人情報保護法第23条が定める安全管理措置（その下位規範である
個人情報保護委員会のガイドラインで「アクセス記録の取得」「監査記録の
保管」が技術的安全管理措置として例示されている）の実装の **一例として**
利用可能な仕組みである。§23 がログ保管そのものを直接要求しているわけ
ではない。同法第24条（従業者監督）・第25条（委託先監督）は §23 安全管理
措置の枠組みの一部として位置付けられ、それぞれ別途の運用要素（選定・
契約・継続的監督）を要する。

弁護士法第23条（秘密保持義務）の履行についても、本ログは「処理の事実
を後追いで確認できる証跡」として運用補助になり得るが、本ログ単体で
これらの法的義務が「履行された」とみなされるわけではない。

詳しい運用上のチェック項目は本プラグインの README に §25 チェックリスト
雛形として別途整理してある。
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="bengo-toolkit 監査ログ（ファイルのメタデータのみ記録、内容は記録しない）",
    )
