# bengo-toolkit 初回インストール スモークテスト runbook

法律事務所への導入時の最初のチェックリスト。新規端末に bengo-toolkit を配置し、基本機能が動作することを 15〜20 分で確認する。
...
- [ ] （任意）`/family-tree` で自己完結 HTML（`.html`）生成
- [ ] `/plugin install bengo-toolkit@llamadrive` で更新（再起動後 /verify）
...
d['plugins'].pop('bengo-toolkit@llamadrive', None)
r.write_text(json.dumps(d, indent=2) + chr(10))
print('removed')
"
```

Claude Code を起動しなおして `/plugin install bengo-toolkit@llamadrive` を再実行する。
