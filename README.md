...
### Claude Cowork で MCP サーバを動かす（agent-format は推奨、他は任意）
...
- `agent-format` — **推奨**。未起動でも `.agent` ファイルは生成できるが、`/family-tree` と `/lawsuit-analysis` のチャット内インライン描画（カンバン・タイムライン・メトリクス等）が機能しない
