...
| 製品 | 対応 | 備考 |
|------|------|------|
| **Claude Code**（CLI・デスクトップアプリ・IDE 拡張） | ✅ 全機能動作 | 主要な想定環境。ターミナルから `claude` コマンドで起動 |
| **Claude Cowork**（Claude Desktop アプリの Cowork モード、v3.3.0〜） | ✅ 動作対応 | プラグイン形式が Claude Code と共通。[Anthropic 公式 Cowork プラグイン開発ガイド](https://github.com/anthropics/knowledge-work-plugins) が同じ `.claude-plugin/plugin.json` + `commands/` + `skills/` + `Bash` tool 許可を記している。Pro / Max / Team / Enterprise プランのみ |
| claude.ai（ブラウザ版 チャット） | ❌ 非対応 | プラグインシステム非搭載。`/plugin install` が存在しない |
| Claude iOS / Android アプリ | ❌ 非対応 | 同上 |
| Claude Desktop アプリ（チャットモード、Cowork モード外） | ⚠ 部分対応 | MCP サーバーのみ手動設定で動作可能だが、本プラグインの主要機能（スラッシュコマンド・監査ログ等）は動作しない |

**対象は「ターミナル／デスクトップ上の AI アシスタントを使う弁護士」**。claude.ai でチャットだけを使っているユーザーには本プラグインは不向き（そもそもインストールできない）。Claude Code は名前に "Code" が付くが**コーディング専用ツールではない**。文書・ファイル・スクリプトを扱う汎用 AI エージェントであり、法律事務所業務（書式入力・戸籍 PDF 解析・訴訟文書分析）にそのまま適用できる。

### Claude Code と Claude Cowork の実行モデルの違い

プラグイン本体はどちらでも動作するが、**実行モデルが大きく異なる** ため、Cowork で使う場合は特有の作法が必要。選択は以下の表を参考にしてほしい。

| 項目 | Claude Code | Claude Cowork |
|------|-------------|----------------|
| 実行場所 | ホスト OS 上で直接実行 | Apple 仮想マシン（VZVirtualMachine）内で実行、ハードウェアレベルの隔離 |
| ファイルアクセス | 既定で全ファイル読取可。Bash sandbox は opt-in | **事前に選択したフォルダのみ**。選択外は読めない |
| `~/.claude-bengo/`（HMAC 鍵・初回案内フラグ） | そのまま永続化 | Cowork の VM は毎セッション初期化される。ホスト側 `~/.claude-bengo/` を**明示的に "Select folders" で選ばない限り**、HMAC 鍵と初回案内フラグは毎回リセットされる |
| 会話の途中で別案件フォルダへ切替 | `cd ~/cases/別案件` で即可能 | 不可。新しいセッションを開始して別フォルダを選択し直す（※1） |
| 会話の途中で任意パスの PDF を開く | パスを貼るだけで読める | 事前選択フォルダ外のファイルはチャット開始時に添付が必要 |
| プラットフォーム | macOS / Linux / Windows / WSL2 | Apple Silicon（M1〜M4）のみ、Windows は家ディレクトリ外の選択制限あり |
| 対応プラン | 全プラン | Pro / Max / Team / Enterprise のみ |

※1 2026 年 2 月までは Cowork でもチャット中にフォルダを追加選択できたが、現在は削除されている。再追加の要望は [anthropics/claude-code#25163](https://github.com/anthropics/claude-code/issues/25163) で追えるが、執筆時点では restore されていないため「事前選択のみ」を前提に運用すること。

### Claude Cowork で本プラグインを使う場合の必須セットアップ

1. **案件フォルダと `~/.claude-bengo/` の両方を Cowork の "Select folders" で事前に選ぶ**
   - 案件フォルダを選ばないと `/family-tree` 等が戸籍 PDF を読めない
   - `~/.claude-bengo/` を選ばないと監査 HMAC 鍵がセッション毎にリセットされる
2. **処理する全ての PDF・XLSX・DOCX は、選択済みフォルダ内に置く**
   - 会話の途中で「このファイルも処理して」と別パスを渡すことはできない
   - どうしても外部ファイルが必要なら、先にファイル添付（drag & drop）で同梱する
3. **別の案件に移るときは新しいセッションを開き、選択フォルダを切り替える**
   - 1 セッション = 1 案件。ミックスしないほうが監査ログが混線しにくい

### Claude Cowork で MCP サーバを動かす（すべて任意）

Cowork は Apple 仮想マシン内で動作するため、プラグインの `.mcp.json` に書かれた `npx` 起動の stdio MCP サーバは VM 内で spawn できない。このため、以下の MCP は**プラグインインストールだけでは Cowork 上で起動しない**:

- `agent-format` — 任意。`/family-tree` と `/lawsuit-analysis` の成果物は描画エンジンを内蔵した自己完結 HTML で生成されるため、未起動でも完結する（ダブルクリックで開け・⌘P で PDF 化できる）。本 MCP はチャット内インライン描画（カンバン・タイムライン・メトリクス等）を追加するだけ
- `xlsx-editor` — 任意。未起動でも Claude が `openpyxl` を都度書いて代替する（呼出毎にコード生成するためやや遅い）
- `docx-editor` — 任意。未起動でも `python-docx` で代替する

Cowork でこれらを動かすには、**Claude Desktop の `claude_desktop_config.json` に MCP サーバ定義を追加する**。Desktop SDK bridge が `"type": "sdk"` として VM 内に自動 proxy する（MCP Apps の UI リソースも保持されるため、`.agent` のインライン描画はそのまま機能する）。

#### かんたんな方法（Claude Code に頼む）
...
プラグイン本体の機能面での差は本質的に無い。ただし**実行環境の制約から以下の挙動差がある**:

| 機能 | Claude Code | Claude Cowork | 差異理由 |
|------|-------------|----------------|-----------|
| `/family-tree`, `/typo-check`, `/lawsuit-analysis`, `/template-fill`, `/template-create` | 任意パスの PDF / DOCX / XLSX を即読取、3 MCP がそのまま起動 | 選択済みフォルダ内に限定。`/family-tree` と `/lawsuit-analysis` のチャット内インライン描画は `agent-format` MCP を desktop-config に追加して bridge した場合のみ機能する（前節参照） | VM mount 範囲 + stdio MCP 制約 |
| `/inheritance-calc` 等の全 7 計算器 | 数値入力のみのため完全同等 | 完全同等 | ファイル I/O 無し |
| `/law-search`（e-Gov API） | 完全同等 | 完全同等 | ネット経由、VM でも API 到達可 |
| `/template-promote`（PII code-gate） | 完全同等 | 完全同等 | 対象 XLSX が選択フォルダ内なら同じ |
| 監査ログ（`./.claude-bengo/audit.jsonl`） | セッション横断で永続化 | 案件フォルダが mount されていれば永続化、そうでなければ VM ephemeral | VM の書込はセッション終了で消える |
| グローバル設定（`~/.claude-bengo/global.json`、HMAC 鍵・初回案内フラグ） | 永続化 | **`~/.claude-bengo/` mount 必須**、mount なしでは毎回初期化 | VM の `~/` は ephemeral |
| `outputs/` ディレクトリの `{template}_filled_YYYYMMDD.xlsx` | 案件フォルダ直下に保存 | 案件フォルダが mount されていれば同じ場所に保存 | 同上 |

**要旨**: Claude Code は「弁護士が日常使いするデスクトップ AI」として想定された主要環境。Claude Cowork は**「選択フォルダを一度決めれば会話中はそこで完結する業務」**向き（例: 1 案件を 1 セッションで一通り処理する）。複数案件を flip しながら進めるような使い方は Claude Code のほうが楽。

### 必要なもの

- **Claude Code または Claude Cowork**（いずれか） — Anthropic の AI アシスタント製品（[Claude Code ダウンロード](https://claude.ai/code) / [Claude Cowork 概要](https://claude.com/product/cowork)）
- **Node.js 18以上** — 内部 MCP サーバの実行に必要（[ダウンロード](https://nodejs.org/)）
- **Python 3.8以上** — 決定論的計算（相続分）・法令検索・監査ログに必要

Git はインストール不要（プラグイン管理は Claude Code / Cowork が内蔵する git 機構を使う）。
...
Claude Code または Claude Cowork を起動し、以下を **Claude 内で** 実行する（ターミナルでの `git clone` は不要）:
...
Claude Code / Cowork が GitHub からリポジトリを取得し、自動的にプラグインとして登録する。Claude Code では再起動後 3 つの MCP サーバ（`xlsx-editor` / `docx-editor` / `agent-format`）が起動する（初回 30 秒程度）。**Cowork では `.mcp.json` の stdio MCP は VM 内で起動しないため、別途デスクトップ設定が必要**（前節「Claude Cowork で MCP サーバを動かす」を参照。Code タブから Claude に頼めば 30 秒で完了する）。

更新は同じく `/plugin install bengo-toolkit@llamadrive` を実行する（marketplace.json の最新バージョンを取得）。
...
```bash
# macOS / Linux / WSL2
git clone https://github.com/llamadrive/bengo-toolkit.git ~/bengo-toolkit-dev
```

Claude Code 内で:

```
/plugin marketplace add ~/bengo-toolkit-dev
/plugin install bengo-toolkit@llamadrive
```
...
バージョンはピン留めされており、更新は Claude Code 標準の `/plugin install bengo-toolkit@llamadrive` 経由で行う（marketplace.json の `version` を見て最新版をキャッシュへ展開）。初回起動時は npx キャッシュがないため各 MCP の初回取得に 30 秒程度かかる（以降はキャッシュから即起動）。
...
for d in ~/cases/*/; do
  ...
  if [ -f "$d/.claude-bengo/audit.jsonl" ]; then
    (cd "$d" && python3 ~/.claude/plugins/cache/llamadrive/bengo-toolkit/{VERSION}/skills/_lib/audit.py export --format csv) \
...
動作確認の次は `QUICKSTART.md` を開く。相続・離婚・交通事故の 3 シナリオで全 24 コマンドに触れる 30 分程度のウォークスルーを用意している。フィードバック窓口も同ドキュメントに記載。

### 覚えるコマンドは 3 つだけ

全 24 コマンドを覚える必要はない。以下のいずれかから始めれば自然に全機能に触れる:
...
```bash
python3 ~/.claude/plugins/cache/llamadrive/bengo-toolkit/{VERSION}/skills/_lib/audit.py export --format csv --since 2026-04-01 > audit-2026-04.csv
```
...
Claude Code 内で `/plugin` → **Marketplaces タブ** → llamadrive を選択 →
...
```
/plugin marketplace update llamadrive         ← catalogue を refresh
/plugin uninstall bengo-toolkit@llamadrive  ← 削除
/plugin install bengo-toolkit@llamadrive    ← 最新版で再取得
